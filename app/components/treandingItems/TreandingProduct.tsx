...
function TreandingProduct(props: BestsellingProps) {
  ...
  const sepratedPrice = price
    ? seprateNumbers(price)
    : seprateNumbers(15651651);

  const dispatch = useDispatch<AppDispatch>();
  const addToCartHandler = () => {
    const productId=link.split('?id=')[1]
    const payloadProductObject={id:productId, name:title, count:1, price:price?price:6546,image}
    dispatch(addItem(payloadProductObject))
  };
  return (
    <div className="treanding-product px-3 my-4 w-full md:w-1/3 xxl:w-1/4 h-auto relative">
      ...
      <div
        ...
        className="cursor-pointer absolute top-3 left-7 text-custom-white bg-custom-main px-1 py-1 rounded-lg transition-colors duration-300 hover:bg-custom-textSecondary"
      >
        <svg
          className=" text-base fill-custom-white"
          focusable="false"
          aria-hidden="true"
          viewBox="0 0 24 24"
